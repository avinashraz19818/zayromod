...
function ensureAudioGate(html) {
  ...
  const snippet = [
    '<script>',
    '/* ZAYRO AUDIO GATE V5 — auto-injected at build time (template-agnostic) */',
    '(function(){',
    '  var __g={on:false, played:false, regOn:false, regAt:0, homeTicks:0};',
    ...
    '      /* ── REGISTER PAGE ENTRY → turant register UI + 1 sec me register.mp3 ── */',
    '      if(isReg){',
    '        if(!__g.regOn){',
    '          __g.regOn=true;',
    '          var now=Date.now();',
    ...
    '      /* ── HOME SETTLE counter (no-balance games ke liye) ── */',
    '      if(isReg||isLogin){ __g.homeTicks=0; }',
    '      else { __g.homeTicks++; }',
    '      /* ── LOGIN GATE ── */',
