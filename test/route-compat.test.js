...
// ── 6. DOM route sniffer: location.href fail ho to DOM se wingo/home detect ──
test('DOM sniffer: iframe DOM se wingo aur home route push hota hai', { skip: skipJsdom }, async () => {
  const { JSDOM } = require('jsdom');
  const page = injectRouteCompat(`<!DOCTYPE html><html><head><title>t</title></head><body>
    <iframe id="gameFrame"></iframe>
    <script>
      var REGISTER_URL="https://dhaniwin.example/register?inviteCode=AB";
      window._kinds=[];
      window.setUrl=function(url){ window._kinds.push(String(url)); };
    </script>
  </body></html>`);
  const dom = loadDom(page);
  const w = dom.window;
  await sleep(150);
  const gf = w.document.getElementById('gameFrame');

  // Device jaisa: location.href read fail/empty, lekin contentDocument readable
  const setDoc = (html) => {
    const frag = new JSDOM(html).window.document;
    Object.defineProperty(gf, 'contentWindow', { configurable: true, get: () => ({ location: { href: '' }, document: frag }) });
    Object.defineProperty(gf, 'contentDocument', { configurable: true, get: () => frag });
  };

  setDoc('<body><div class="bet-box">WinGo 30s</div><div class="period">20260</div></body>');
  await sleep(1400);
  assert.ok(w._kinds.some((u) => /#\/(saasLottery|wingo)/i.test(u)), `wingo push nahi hua: ${JSON.stringify(w._kinds)}`);

  setDoc('<body><div>Featured Games</div><div>Top Games</div></body>');
  await sleep(1400);
  assert.ok(w._kinds.some((u) => /#\/home/i.test(u)), `home push nahi hua: ${JSON.stringify(w._kinds)}`);
  dom.window.close();
});
